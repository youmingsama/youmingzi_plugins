...
func _on_load():
	...
	register_keyword("来点miku",anime_pic,{},MatchMode.INCLUDE)
...
func anime_pic(keyword,parsed,arg,event):
	var _result:HttpRequestResult = await Utils.send_http_get_request("https://hatsunemiku-tov.imwork.net/api/miku")
	var _dic = _result.get_as_dic() #从返回结果中解析并获取字典
	if !_dic.is_empty(): #判断是否成功获取到包含数据的字典(不为空)
		var reply:String = _dic["url"] #从字典中获取图片链接
		reply = reply.replace("cat","re")
		var msg = ImageMessage.init_url(reply) #根据图片链接构建一个图片消息ImageMessage类实例
		#将图片消息发送回触发此关键词的位置，并引用原消息(第二个参数)；且如果来自群聊，则AT原消息发送者(第三个参数)
		event.reply(msg,true,true)
