...
func _on_load():
	...
	register_keyword(["幽明子","{@}"],_chat,{},MatchMode.INCLUDE)

	#注册群消息事件与好友消息事件，绑定到内置的"trigger_keyword"函数用于触发关键词，并将优先级设置为1
	#注意：若要直接绑定到内置函数，则必须附带双引号；仅消息事件可用于触发关键词
	register_event([GroupMessageEvent,FriendMessageEvent],["respond_context","trigger_keyword"],5)
...
func chat(arg,event):
	...
	if !_dic.is_empty(): #判断是否成功获取到包含数据的字典(不为空)
		...
		reply = reply.replace("菲菲","幽明子").replace("{br}","\n")
		#将获取到的文本发送回触发此关键词的位置，并引用原消息(第二个参数)；且如果来自群聊，则AT原消息发送者(第三个参数)
